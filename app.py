...
# Fetch data from TCL support website
def fetch_support_data(query):
    url = f"https://support.tcl.com/us/search?q={query}"
    response = requests.get(url)
    if response.status_code == 200:
        soup = BeautifulSoup(response.content, "html.parser")
        faqs = soup.find_all("div", class_="faq-content")
        return [faq.get_text(strip=True) for faq in faqs]
...
def troubleshoot(error_code, data):
    ...
    if not row.empty:
        ...
        # Fetch online data as fallback
        faqs = fetch_support_data(error_code)
        return "\n".join(faqs)
